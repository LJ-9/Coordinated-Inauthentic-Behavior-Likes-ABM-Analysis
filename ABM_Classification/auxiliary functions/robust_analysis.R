...
# does not report warning if glm logisitc regression does not converge, since this is dealt with in the code (the lasso/glmnet is used in that case, which is documented, too)
...
clusters <- function(dataset, vrounds, threshold_u,size){
  n_agents=size+900
  # size specifies number of genuine agents
  ...
  dataset = dataset[,c(1:(size+13),1001:1900)]
  # dim(dataset)
  ...
  labmat_gmm = data.frame(matrix(ncol=(size+900)))
  labmat_km = data.frame(matrix(ncol=(size+900)))
  labmat_gmm_entr = data.frame(matrix(ncol=(size+300)))
  labmat_km_entr = data.frame(matrix(ncol=(size+300)))
  gap_fail <- c()
  gap_fail_entr <- c()
  lasso_gmm_yes <-c()
  lasso_km_yes <- c()
  ...
  for (h in 1:length(matlist)) {
    ...
    confusion <- function(cls, n_agents= size+900, b_pct=(900/(size+900)), pct=TRUE){ # same as confusion function but allows for up to 4 classified groups
      # Set the true classes of each agent
      tru = c(rep(1,size),rep(2,900))
    ...
    confusion_entr <- function(cls, n_agents= (size+300), b_pct=(300/(size+300)), pct=TRUE){ # same as confusion function but allows for up to 4 classified groups
      # Set the true classes of each agent
      tru = c(rep(1,size),rep(2,300))
      ...
      if(pct){ # return in percent instead of actual numbers
        confusion = apply(confusion,2,function(x) x/(c((1-b_pct),b_pct)*(size+300)))
      }
      return(confusion)
    }
    ...
    dim1 = dim_plot(1,s, ncol(dataset), (900/(size+900)), TRUE) 
    
    # legend("top", agenttypes ,pch=19, title = "Agent Types",
    # col=add.alpha(colb,.6), bty='n')
    
    
    dim2 = dim_plot(2,s, ncol(dataset), (900/(size+900)), TRUE)
    #legend("top", agenttypes ,pch=19, title = "Agent Types",
    ...
    # Decomposition
    M = cor(dataset[,1:(300+size)]) # Recalculate correlation matrix for subset of data now
    s = svd(M)                # Singular value decomposition
    dim1 = dim_plot(1,s, ncol(dataset[,1:(300+size)]), (300/(300+size)), TRUE) 
    dim2 = dim_plot(2,s, ncol(dataset[,1:(300+size)]), (300/(300+size)), TRUE) 
    
    # # 1
    # source(km_entr_f_1)#("kmeans_entropy_1.R")
    # classification_entr_1 <- km_entropy_1(dataset,dim1,dim2,n_agents,vrounds)
    # confusion_matrix_entr_1 <- confusion_entr(classification_entr_1)
    
    # 2
    
    source(here("ABM_Classification/auxiliary functions/robust_test_km_entr_3.R"))
    source(here("ABM_Classification/auxiliary functions/robust_tests_gmm_entropy_2.R"))
    
    
    
    classification_entr_1 <- gmm_entropy_2(dataset,dim1,dim2,n_agents,size)
    
    labmat_gmm_entr[h,] <- classification_entr_1
    
    # 3
   # source(km_entr_f_3)#("kmeans_entropy_3.R")
    classification_entr_2 <- km_entropy_3(dataset,dim1,dim2,n_agents,size)
