...
gmm_entropy_2 <- function(dataset,dim1,dim2,n_agents){
  ...
  nef <- c(group_1,group_2,group_3,group_4)
  ...
  #check which labels are present in cluster (sometimes some dont make it for classification due to low probability, then e run into troubles later)
  ...
  source(here("ABM_Classification/auxiliary functions/entropy_new.R"))#("entropy_new.R")
  ...
  # source(entr_)#("entropy_new.R")
  # chaos <- En(cl, dataset,classification,vrounds)
  # # get names when 
  # 
  # # create newlybefore entering loop
  # single <- data.frame(matrix(ncol = 2, nrow = 0))
