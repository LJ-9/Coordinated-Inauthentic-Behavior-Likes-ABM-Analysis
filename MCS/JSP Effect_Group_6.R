...
# CHANGE HERE
load(here("Data/group_6_mean_100_gen_agents_gmm.Rda"))
load(here("Data/group_6_mean_100_gen_agents_km.Rda"))
load(here("Data/group_6_sd_100_gen_agents_gmm.Rda"))
load(here("Data/group_6_sd_100_gen_agents_km.Rda"))
...
gmm.list <- function(parameter){
  ...
  groups <- list(group_1[1:100], group_6)
  names(groups) <- c("Genuine", "AQlow")
...
#
## Best case test:
# gmm.list(3)$gmm.mean[1] - gmm.list(3)$gmm.sd[1]  # GMM param 3 mean misclass % of Genuine - SD for same
# gmm.list(3)$best[1] : best case for agent group 1 in parameter 3.

# gmm.list(3)$gmm.mean[2] - gmm.list(3)$gmm.sd[2]  
# gmm.list(3)$best[2] : best case for nefarious agents under consideration in parameter 3.
...
my.untrimmed.jury[[1]] <- c(group_1[1:100], group_6)
...
save.file <- here(paste0("MCS/JSP Effect/Effects_Group_6_", Sys.Date(), ".Rda"))
...
#     Nothing happens here: Output matrices ----
#gmm.correct#; gmm.sd
#km.correct#; km.sd
...
gmm.jury.correctness <- list("GMM Jury Correctness, Gen+AQlow, 500 Rounds" = gmm.correct, "SD" = round(gmm.sd,2) )
gmm.jury.correctness.up.and.yes <- list("GMM Jury Correctness, Gen+AQlow, Up and Yes" = gmm.correct.up.and.yes, "SD" = round(gmm.sd.up.and.yes,2) )
km.jury.correctness <- list("KM Jury Correctness, Gen+AQlow, 500 Rounds" = km.correct, "SD" = round(km.sd,2) )
km.jury.correctness.up.and.yes <- list("KM Jury Correctness, Gen+AQlow, Up and Yes" = km.correct.up.and.yes, "SD" = round(km.sd.up.and.yes,2) )
