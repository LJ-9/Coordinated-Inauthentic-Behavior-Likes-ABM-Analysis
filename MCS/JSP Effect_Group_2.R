...
# CHANGE HERE
load(here("Data/group_2_mean_100_gen_agents_gmm.Rda"))
load(here("Data/group_2_mean_100_gen_agents_km.Rda"))
load(here("Data/group_2_sd_100_gen_agents_gmm.Rda"))
load(here("Data/group_2_sd_100_gen_agents_km.Rda"))
...
gmm.list <- function(parameter){
  ...
  groups <- list(group_1[1:100], group_2)
  names(groups) <- c("Genuine", "XYZup")
...
my.untrimmed.jury[[1]] <- c(group_1[1:100], group_2)
names(my.untrimmed.jury) <- c("Untrimmed Jury")

# Seeds and rounds to run analysis on, and place to save results ----
my.seeds <- 1:100  # max 1:100, min 1:2  . Paper used 1:100
my.rounds <- 1:500 # max 1:1000, min 1:2 . Paper used 1:500
# CHANGE HERE
#save.file <- here(paste0("MCS/Effect/Effects_All_Groups_", Sys.Date(), ".Rda"))
save.file <- here(paste0("MCS/JSP Effect/Effects_Group_2_", Sys.Date(), ".Rda"))
...
gmm.jury.correctness <- list("GMM Jury Correctness, Gen+XYZup, 500 Rounds" = gmm.correct, "SD" = round(gmm.sd,2) )
gmm.jury.correctness.up.and.yes <- list("GMM Jury Correctness, Gen+XYZup, Up and Yes" = gmm.correct.up.and.yes, "SD" = round(gmm.sd.up.and.yes,2) )
km.jury.correctness <- list("KM Jury Correctness, Gen+XYZup, 500 Rounds" = km.correct, "SD" = round(km.sd,2) )
km.jury.correctness.up.and.yes <- list("KM Jury Correctness, Gen+XYZup, Up and Yes" = km.correct.up.and.yes, "SD" = round(km.sd.up.and.yes,2) )
...
# Some sanity check ---- :
#gmm.trim <- trimmed.jury(gmm.list(3)$worst, c(1,2))[[1]]
#sum(gmm.trim < 1001)

#km.trim <- trimmed.jury(km.list(3)$worst, c(1,2))[[1]]

# Rasmus started run on Dec. 3, 14:12
# finished 17:05